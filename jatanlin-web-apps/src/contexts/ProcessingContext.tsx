...
export const ProcessingProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const broadcastSetSteps = useCallback((newSteps: Step[]) => {
    setSteps(newSteps);
    broadcast('UPDATE_STEPS', newSteps);
  }, [broadcast]);

  const broadcastSetCurrentStepId = useCallback((id: number) => {
    setCurrentStepId(id);
    broadcast('UPDATE_CURRENT_STEP', id);
  }, [broadcast]);

  const broadcastSetCountdown = useCallback((count: number) => {
    setCountdown(count);
    broadcast('UPDATE_COUNTDOWN', count);
  }, [broadcast]);

  const broadcastSetSessionStartTime = useCallback((time: string | null) => {
  ...
  const broadcastSetVehicleActualId = useCallback((id: string | null) => {
    setVehicleActualId(id);
    broadcast('UPDATE_VEHICLE_ACTUAL_ID', id);
  }, [broadcast]);

  const broadcastSetPhase = useCallback((nextPhase: "init" | "processing") => {
    setPhase(nextPhase);
    broadcast("UPDATE_PHASE", nextPhase);
  }, [broadcast]);

  const broadcastSetAnprData = useCallback((data: any | null) => {
    setAnprData(data);
    broadcast('UPDATE_ANPR_DATA', data);
  }, [broadcast]);

  const broadcastSetWeightData = useCallback((data: any | null) => {
    setWeightData(data);
    broadcast('UPDATE_WEIGHT_DATA', data);
  }, [broadcast]);

  const broadcastSetAxleData = useCallback((data: any | null) => {
    setAxleData(data);
    broadcast('UPDATE_AXLE_DATA', data);
  }, [broadcast]);

  const broadcastSetDimensionData = useCallback((data: any | null) => {
    setDimensionData(data);
    broadcast('UPDATE_DIMENSION_DATA', data);
  }, [broadcast]);

  const broadcastSetCctvData = useCallback((data: any | null) => {
    setCctvData(data);
    broadcast('UPDATE_CCTV_DATA', data);
  }, [broadcast]);

  const broadcastSetVehicleClassData = useCallback((data: any | null) => {
    setVehicleClassData(data);
    broadcast('UPDATE_VEHICLE_CLASS_DATA', data);
  }, [broadcast]);

  const broadcastSetViolationResult = useCallback((result: ViolationResult | null) => {

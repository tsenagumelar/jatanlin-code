...
SET config_value = CASE config_key
  ...
  WHEN 'WEIGHING_TRIGGER_TIMEOUT_SECONDS' THEN '60'
  WHEN 'CCTV_TRIGGER_URL' THEN 'http://cctv-streamer:8090/record'
  WHEN 'CCTV_TRIGGER_SECONDS' THEN '30'
  ELSE config_value
END,
updated_date = now()
WHERE config_key IN (
  ...
  'WEIGHING_TRIGGER_TIMEOUT_SECONDS',
  'CCTV_TRIGGER_URL',
  'CCTV_TRIGGER_SECONDS'
);
SQL

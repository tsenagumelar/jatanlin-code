...
const UNITS: UnitRow[] = [
  {
    ...
    address: "Jl. Tol Dalam Kota KM 12",
    region: "Default",
    lat: -6.19,
    lng: 106.82,
    status: "active",
  ...
  },
  {
    ...
    city: "Bekasi",
    province: "Jawa Barat",
    address: "Tol Jakarta-Cikampek KM 28",
    region: "JABODETABEK",
    lat: -6.4,
    lng: 107.1,
    status: "active",
  ...
  },
  {
    ...
    address: "Jl. Tol Semarang – Solo KM 15",
    region: "JAWA TENGAH",
    lat: -7.0,
    lng: 110.44,
    status: "warning",
  ...
  },
  {
    ...
    city: "Surabaya",
    province: "Jawa Timur",
    address: "Jl. Tol Surabaya – Gresik KM 9",
    region: "JAWA TIMUR",
    lat: -7.25,
    lng: 112.75,
    status: "active",
  ...
  },
  {
    ...
    address: "Jl. Tol Balikpapan – Samarinda KM 3",
    region: "KALIMANTAN",
    lat: -1.27,
    lng: 116.83,
    status: "active",
...
const MAP_SITES: UnitSite[] = UNITS.map((u) => ({
  ...
  officer: u.officer
    ? { name: u.officer.name, badge: u.officer.badge, rank: u.officer.rank }
    : undefined,
}));
...
const ENFORCEMENT_POINTS: EnforcementPoint[] = [
  ...
  },
  {
    ...
    weight: 32.7,
    date: "16 Mei 2026",
    siteName: "JTL004 · Bripka Ignatius W.",
  },
  {
    id: "e7",
    lat: -7.27,
...
const TABS: { id: Tab; label: string; icon: React.ReactNode }[] = [
  {
    id: "units",
    label: "Peta Unit",
    icon: <Map24Regular className="w-4 h-4" />,
  },
  {
    id: "enforcement",
    label: "Peta Penindakan",
    icon: <History24Regular className="w-4 h-4" />,
  },
  {
    id: "operators",
    label: "Operator",
    icon: <People24Regular className="w-4 h-4" />,
  },
  {
    id: "analytics",
    label: "Analitik",
    icon: <DataBarVertical24Regular className="w-4 h-4" />,
  },
];
...
function TrendBadge({ value }: { value: number }) {
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-xs font-semibold ${pos ? "text-green-600" : "text-red-500"}`}
    >
      {pos ? (
  ...
  return (
    <div className="absolute top-3 right-12 z-[1000] flex items-center gap-4 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-xl shadow-md border border-slate-100 text-xs font-semibold">
      <span className="flex items-center gap-1.5 text-green-700">
        <Circle12Filled className="w-2.5 h-2.5 text-green-500" /> Aktif (
        {activeCount})
      </span>
      <span className="flex items-center gap-1.5 text-amber-700">
        <Circle12Filled className="w-2.5 h-2.5 text-amber-500" /> Peringatan (
        {warningCount})
      </span>
      <span className="flex items-center gap-1.5 text-slate-500">
        <Circle12Filled className="w-2.5 h-2.5 text-slate-400" /> Offline (
        {offlineCount})
      </span>
    </div>
  );
...
export function DataCenterModule() {
  ...
  const { data: analyticsData } = useQuery(GET_ANALYTICS, {
    variables: { start: thirtyDaysAgo },
    fetchPolicy: "cache-and-network",
  });
  const { data: operatorsData } = useQuery(GET_OPERATORS, {
    fetchPolicy: "cache-and-network",
  });
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Monitoring terpusat unit Jatanlin
            </p>
      ...
      {showMap && (
        <div className="relative shrink-0" style={{ height: 380 }}>
          <UnitMap
            ...
            onSiteClick={(s) =>
              setSelectedSite(selectedSite === s.id ? null : s.id)
            }
          />
          {/* Legend overlay */}
          {activeTab === "units" && (
      ...
      <div className="flex-1 p-6 space-y-5">
          ...
          <>
            ...
            <div className="flex gap-4">
              ...
              <StatCard
                ...
                value={
                  totalVehicles || UNITS.reduce((a, u) => a + u.totalToday, 0)
                }
                sub="semua unit"
              ...
              <StatCard
                ...
                value={
                  totalViolations || UNITS.reduce((a, u) => a + u.violations, 0)
                }
                sub="0% dari total"
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/60">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left text-[11px] font-bold text-slate-400 uppercase tracking-wide px-4 py-3 whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredUnits.map((unit) => (
                      <tr
                        ...
                        onClick={() =>
                          setSelectedSite(
                            selectedSite === unit.id ? null : unit.id,
                          )
                        }
                      >
                        {/* Unit */}
                        <td className="px-4 py-3.5">
                          ...
                          <p className="text-[10px] text-slate-400 ml-4.5 mt-0.5">
                            {unit.region}
                          </p>
                        </td>
                        {/* Lokasi */}
                        ...
                        <td className="px-4 py-3.5">
                          <p className="text-sm text-slate-700 font-medium">
                            {unit.officer.rank} {unit.officer.name}
                          </p>
                        </td>
                        {/* Status */}
                        <td className="px-4 py-3.5">
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50/40">
                <p className="text-xs text-slate-500">
                  Menampilkan 1 – {filteredUnits.length} dari {UNITS.length}{" "}
                  unit
                </p>
                <div className="flex items-center gap-2">
                  <button
                    className="w-7 h-7 flex items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-400 hover:bg-slate-100 disabled:opacity-40 transition-colors"
                    disabled
                  >
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-slate-50">
              {ENFORCEMENT_POINTS.map((pt) => {
                const colorMap = {
                  OL: "text-red-700 bg-red-50 border-red-200",
                  OD: "text-orange-700 bg-orange-50 border-orange-200",
                  OLOD: "text-purple-700 bg-purple-50 border-purple-200",
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${colorMap[pt.violationType]}`}
                    >
                      {labelMap[pt.violationType]}
                    </span>
                    <span className="font-mono font-bold text-slate-800 text-sm w-24">
                      {pt.plateNo}
                    </span>
        ...
        {activeTab === "operators" && (
          <div className="space-y-4">
              ...
              {[
                {
                  ...
                  value: operators.length || UNITS.length,
                  icon: <People24Regular className="w-5 h-5 text-blue-600" />,
                  iconBg: "bg-blue-50",
                },
                {
                  label: "Operator Aktif",
                  value:
                    operators.filter((o: { is_active: boolean }) => o.is_active)
                      .length || UNITS.length,
                  icon: (
                    <CheckmarkCircle24Regular className="w-5 h-5 text-green-600" />
                  ),
                  iconBg: "bg-green-50",
                  sub: "",
                },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {(operators.length > 0 ? operators : UNITS).map(
                (
                  item:
                    | UnitRow
                    | {
                        ...
                        email?: string;
                        is_active: boolean;
                        master_role: { role_name: string };
                      },
                ) => {
                  const isUnit = "officer" in item;
                  const name = isUnit
                    ? `${(item as UnitRow).officer.rank} ${(item as UnitRow).officer.name}`
                    : (item as { full_name: string }).full_name;
                  const badge = isUnit
                    ? (item as UnitRow).officer.badge
                    : ((item as { badge_no?: string }).badge_no ?? "—");
                  const role = isUnit
                    ? `Unit ${(item as UnitRow).code}`
                    : ((item as { master_role: { role_name: string } })
                        .master_role?.role_name ?? "—");
                  const email = isUnit
                    ? `${(item as UnitRow).code}@jatanlin.polri.go.id`
                    : ((item as { email?: string }).email ?? "");
                  const active = isUnit
                    ? (item as UnitRow).status === "active"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? "bg-green-500" : "bg-slate-300"}`}
                          />
                        </div>
        ...
        {activeTab === "analytics" && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
              ...
              <div className="xl:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <p className="text-sm font-bold text-slate-800 mb-4">
                  Tren Kendaraan 7 Hari Terakhir
                </p>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <p className="text-sm font-bold text-slate-800 mb-4">
                Perbandingan Kendaraan per Unit (hari ini)
              </p>
              <ResponsiveContainer width="100%" height={180}>
                <BarChart
                  data={UNITS.map((u) => ({
                    name: u.code,
                    kendaraan: u.totalToday,
                    pelanggaran: u.violations,
                  }))}
                  margin={{ top: 4, right: 8, left: -20, bottom: 0 }}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />

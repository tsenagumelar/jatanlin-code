...
public sealed class NatsCacheService : INatsCacheService, IDisposable
{
        ...
        public NatsCacheService(ILogger<NatsCacheService> logger, IOptions<NatsOptions> options)
    {
        ...
        try
        {
            ...
            EnsureKeyValueBucket(_connection, _bucket);
    ...
    private void EnsureKeyValueBucket(IConnection connection, string bucket)
    {
        var management = connection.CreateKeyValueManagementContext();
        try
        {
        ...
        {
            var config = KeyValueConfiguration.Builder(bucket).Build();
            management.Create(config);
            _logger.LogInformation("Created NATS KV bucket {Bucket}", bucket);
        }
    }

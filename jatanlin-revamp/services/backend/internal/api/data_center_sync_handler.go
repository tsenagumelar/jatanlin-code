...
func (s *Server) TriggerDataCenterSyncRange(c *fiber.Ctx) error {
	...
	err = s.DB.QueryRowContext(context.Background(), `
		WITH actual_scope AS MATERIALIZED (
			SELECT
				id,
				site_id,
				session_id,
			...
			WHERE COALESCE(is_deleted, false) = false
			  AND COALESCE(created_date, updated_date, now()) >= $1
		...
		u_status AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.transact_vehicle_actual_id = s.id
			RETURNING 1
		),
		u_anpr AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.id = s.anpr_id OR (s.session_id IS NOT NULL AND t.session_id = s.session_id)
			RETURNING 1
		),
		u_axle AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.id = s.axle_id OR (s.session_id IS NOT NULL AND t.session_id = s.session_id)
			RETURNING 1
		),
		u_cctv AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.id = s.transact_cctv_id OR (s.session_id IS NOT NULL AND t.session_id = s.session_id)
			RETURNING 1
		),
		u_dimension AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.id = s.transact_dimension_id OR (s.session_id IS NOT NULL AND t.session_id = s.session_id)
			RETURNING 1
		),
		u_weighing AS (
			...
			SET updated_date = now()
			FROM actual_scope s
			WHERE t.id = s.transact_weighing_id OR (s.session_id IS NOT NULL AND t.session_id = s.session_id)
			RETURNING 1
		),
		u_session AS (
